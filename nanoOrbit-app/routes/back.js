...
router.post('/satellites/:id/statut', checkAuth, checkRole(['operateur', 'admin']), async (req, res) => {
  try {
    ...
    await connection.query('UPDATE SATELLITE SET statut_operationnel = ? WHERE id_satellite = ?', [statut, id]);
    connection.release();
...
router.post('/participations', checkAuth, checkRole(['responsable', 'admin']), async (req, res) => {
  try {
    ...
    const [missions] = await connection.query('SELECT date_fin FROM MISSION WHERE id_mission = ?', [id_mission]);
...
router.post('/satellites/:id/deorbit', checkAuth, checkRole(['admin']), async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    await connection.query('UPDATE SATELLITE SET statut_operationnel = ? WHERE id_satellite = ?', ['Désorbité', id]);
    connection.release();
...
router.get('/satellites-operationnels', checkAuth, async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT id_satellite, nom FROM SATELLITE WHERE statut_operationnel = "Opérationnel"');
    connection.release();
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.get('/stations-actives', checkAuth, async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT id_station, nom FROM STATION_SOL WHERE etat = "Active"');
    connection.release();
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
router.get('/missions-actives', checkAuth, async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT id_mission, nom FROM MISSION WHERE date_fin IS NULL OR date_fin > NOW()');
    connection.release();
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
